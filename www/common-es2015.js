...
let FicheService = class FicheService {
    ...
    getFiche(id) {
        return this.storage.get('fiche' + id).then((val) => val).catch(this.handleError);
    }
    sendFiche(fiche) {
    ...
    create(fiche, callback) {
        this.storage.ready().then(() => {
            this.storage.get('i').then(val => {
                const i = val + 1; // set l'id de la fiche
                fiche.id = i;
    ...
    presentAlert(title, subTitle, ficheClean, isSended) {
        return Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"])(this, void 0, void 0, function* () {
...
let UtilsService = class UtilsService {
    ...
    calculTime(ficheForm) {
        if (ficheForm.get('depart').value !== '' && ficheForm.get('arrive').value !== '') {
            ...
            let minutes = Math.floor(diff / (1000 * 60)) - (heures * 60);
            heures = heures ? heures + 'h' : '';
            minutes = this.formatTempsPasse(minutes, 'm');
            return heures + minutes;
        }
    }
    formatTempsPasse(dataTime, initial) {
        let unit = dataTime;
        if (unit === 0) {
            unit = '';
        }
